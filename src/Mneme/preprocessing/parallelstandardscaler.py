...
from Mneme.utils import reduce_std_scalers, _copy_attr
...
class ParStandardScaler(StandardScaler):
    '''
    ParStandardScaler is a parallelized version of the StandardScaler from sklearn.preprocessing. 
    It scales features in a dataset to have zero mean and unit variance. 
    ...
    def __init__(self, data_file: str, num_idxs: List[str], copy: Optional[bool] = True, 
                 with_mean: Optional[bool] = True, with_std: Optional[bool] = True) -> None:
        '''
        Initialize the ParStandardScaler object.

        This constructor method initializes the ParStandardScaler object by calling the constructor of the superclass 
        (StandardScaler) with the provided parameters and initializing additional attributes specific to the 
        ParStandardScaler class.
        ...
        Args:
            ...
            copy (bool, optional): If True, the original data is copied and scaled. If False, try to avoid a copy 
            and scale in place. This is not guaranteed to always work in place (e.g. if the data is
            a numpy array with an int dtype, a copy will be returned even with copy=False). Defaults to True.
            with_mean (bool, optional): Whether to center the data before scaling (default is True).
            with_std (bool, optional): Whether to scale the data to unit variance (default is True).
        '''
        
        # Call the constructor of the superclass (StandardScaler)
    ...
    def process_chunk(self, chunk_data: pl.DataFrame) -> None:
        '''
        Fits a StandardScaler to a chunk of data and stores it.

        This method takes a chunk of data, fits a StandardScaler to the features specified by num_idxs 
        ...
        # If 'with_mean' is False, set the 'with_mean' attribute of the partial fitted scaler to False
        # This is done after fitting because the mean is used in the computation of the incremental variance.
        # If the partial fitted scaler was called with the 'with_mean' attribute set to False and 'with_std' was True, 
        # the mean would be None and the variance could not be computed. Therefore, the mean is always computed and 
        # if 'with_mean' is set to False, then after the final reduction in the 'reduce_std_scalers' function, 
        # the corresponding 'mean_' field of the final fitted standard scaler is set to None.
        
        scaler.with_mean = self.with_mean
        
        scaler.with_std = self.with_std
    ...
    def reduce(self) -> None:
        ...
        This method uses the reduce_std_scalers function to combine the scalers fitted to each chunk of data 
        into a final scaler and copies the attributes of the final scaler to the current ParStandardScaler object. 
        ...
        final_scaler = reduce_std_scalers(self.scalers_)
        
        # Copy the attributes of the final scaler to the ParStandardScaler object
    ...
    def get_partial_fit(self) -> StandardScaler:
        '''
        ...
        Returns:
            tmp_scalers (StandardScaler): The partial fitted scaler.
        '''
    ...
    def set_partial_fits(self, partial_fits: List[StandardScaler]) -> None:
        '''
        Gather the partial fitted scalers (from the different chunks of data) into the 'scalers_' list.
        
        Args:
            partial_fits (List[StandardScaler]): A list of partial fitted scalers.
    ...
    def print(self, use_parallel: bool = True) -> None:
        ...
        kind_scaler = "pstdscaler" if use_parallel else "stdscaler"
    ...
    def _fit(self, use_parallel: Optional[bool] = False, block_reader: Union[Optional[BlockReader], None] = None, 
        ...
        if not use_parallel:
            ...
            t2 = time()
            _Mneme_logger.benchmark(f"Sequential Times of Standalone StandardScaler -> Fit: {t1-t0:.6f} | Reduce: {t2-t1:.6f}")
            ...
            t2 = time()
            _Mneme_logger.benchmark(f"Parallel Times of Standalone StandardScaler -> Fit: {t1-t0:.6f} | Reduce: {t2-t1:.6f}")
            
            
    def _partial_fit(self, index: int, args: tuple) -> StandardScaler:
        '''
        Perform fitting on a specific block of data.

        This method loads a block of data, fits a StandardScaler on it and returns the fitted scaler. The block of data 
        ...
        Returns:
            StandardScaler: The scaler fitted to the block of data.
        '''
        ...
        if block_offsets[index] == block_offsets[0]:
            _Mneme_logger.benchmark(f"[INFO] Indicative time for loading the data of a block [Standalone ParStandardScaler]: {t1-t0:.6f}")
        
        # Fit a StandardScaler on the loaded chunk and return the scaler
        scaler = StandardScaler(copy = self.copy)
        ...
        # This is done after fitting because the mean is used in the computation of the incremental variance.
        # If the partial fitted scaler was called with the 'with_mean' attribute set to False and 'with_std' was True, 
        # the mean would be None and the variance could not be computed. Therefore, the mean is computed and 
        # if 'with_mean' is set to False, then after the final reduction in the 'reduce_std_scalers' function, 
        # the corresponding 'mean_' field of the final fitted standard scaler is set to None.
    ...
    def _load_chunk(self, training_file: str, block_size: int, columns: List[str], num_idxs: List[str], 
        ...
        def _compute_features(imputer: ParImputer, num_idxs: List[str], columns: List[str]) -> List[str]:
            '''
            This function determines the union of features from the ParImputer object and the num_idxs of the ParStandardScaler 
            ...
            Returns:
                List[str]: A list containing the union of features from the ParImputer and ParStandardScaler in the order they appear.
            '''
            ...
            ordered_list = []
            # Create a list containing the union of features from the ParImputer and ParStandardScaler 
