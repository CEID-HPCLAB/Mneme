...
class ParMinMaxScaler(MinMaxScaler):
    ...
    def __init__(self, data_file: str, num_idxs: List[str], feature_range: Optional[tuple] = (0,1), 
                 copy: Optional[bool] = True, clip: Optional[bool] = False) -> None:
        '''
        Initializes the ParMinMaxScaler object.

        Args:
            data_file (str): The path to the data file to be scaled.
            num_idxs (List[str]): The numerical indices of the features to be scaled.
            feature_range (tuple, optional): The desired range of transformed data. Defaults to (0,1).
            ...
            clip (bool, optional): If True, the data is clipped after scaling (to provided `feature range`). Defaults to False.
        '''
        
        # Call the constructor of the superclass (MinMaxScaler)
    ...
    def process_chunk(self, chunk_data: pl.DataFrame) -> None:
        ...
        This method takes a chunk of data, fits a MinMaxScaler to the features specified by num_idxs 
    ...
    def reduce(self) -> None:
        ...
        This method uses the reduce_minmax_scalers function to combine the scalers fitted to each chunk of data 
        into a final scaler and copies the attributes of the final scaler to the current ParMinMaxScaler object. 
        ...
        # Copy the attributes of the final scaler to the ParMinMaxScaler object
    ...
    def _fit(self, use_parallel: Optional[bool] = False, block_reader: Union[Optional[BlockReader], None] = None, 
        ...
        if not use_parallel:
            ...
            t2 = time()
            _Mneme_logger.benchmark(f"Sequential Times of Standalone MinMaxScaler -> Fit: {t1-t0:.6f} | Reduce: {t2-t1:.6f}")
            ...
            t2 = time()
            _Mneme_logger.benchmark(f"Parallel Times of Standalone MinMaxScaler -> Fit: {t1-t0:.6f} | Reduce: {t2-t1:.6f}")
    ...
    def _partial_fit(self, index: int, args: tuple) -> MinMaxScaler:
        ...
        This method loads a block of data, fits a MinMaxScaler on it and returns the fitted scaler. The block of data 
        ...
        # Fit a MinMaxScaler on the loaded chunk and return the scaler
        scaler = MinMaxScaler(feature_range = self.feature_range, copy = self.copy, clip = self.clip)
    ...
    def _load_chunk(self, training_file: str, block_size: int, columns: List[str], num_idxs: List[str], 
        ...
        def _compute_features(imputer: ParImputer, num_idxs: List[str], columns: List[str]) -> List[str]:
            ...
            ordered_list = []
            # Create a list containing the union of features from the ParImputer and ParMinMaxScaler 
