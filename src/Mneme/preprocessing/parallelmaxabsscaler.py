...
class ParMaxAbsScaler(MaxAbsScaler):
    ...
    def __init__(self, num_idxs: List[str], data_file: str = None, copy: Optional[bool] = True) -> None:
    ...
    def fit(self, block_reader: Union[Optional[BlockReader], None] = None, num_workers: Optional[int] = 2, IO_workers: Optional[int] = 1, 
        ...
        if num_workers == 1 and IO_workers == 1:
            ...
            t2 = time()
            _Mneme_logger.benchmark(f"Sequential Standalone MaxAbsScaler Time -> Fit: {t1-t0:.6f} | Reduce: {t2-t1:.6f}")
            ...
            t2 = time()
            _Mneme_logger.benchmark(f"Parallel Standalone MaxAbsScaler Time -> Fit: {t1-t0:.6f} | Reduce: {t2-t1:.6f}")
