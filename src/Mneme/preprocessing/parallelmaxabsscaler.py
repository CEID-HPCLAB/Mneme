...
from Mneme.utils import reduce_maxabs_scalers, _copy_attr
...
class ParMaxAbsScaler(MaxAbsScaler):
    '''
    ParMaxAbsScaler is a parallelized version of the MaxAbsScaler from sklearn.preprocessing.  
    It scales the data to lie within the range [-1, 1] by dividing each feature by its maximum absolute value.
    ...
    def __init__(self, data_file: str, num_idxs: List[str], copy: Optional[bool] = True) -> None:
        '''
        Initialize the ParMaxAbsScaler object.

        This method initializes the ParMaxAbsScaler object by calling the constructor of the superclass 
        (MaxAbsScaler) with the provided parameters and initializing additional attributes specific to the 
        ParMaxAbsScaler class.
        ...
        # Call the constructor of the superclass (MaxAbsScaler)
        super(ParMaxAbsScaler, self).__init__(copy = copy)
    ...
    def process_chunk(self, chunk_data: pl.DataFrame) -> None:
        '''
        Fits a MaxAbsScaler to a chunk of data and stores it.

        This method takes a chunk of data, fits a MaxAbsScaler to the features specified by num_idxs 
        ...
        scaler = MaxAbsScaler(copy = self.copy)
    ...
    def reduce(self) -> None:
        ...
        This method uses the reduce_maxabs_scalers function to combine the scalers fitted to each chunk of data 
        into a final scaler and copies the attributes of the final scaler to the current ParMaxAbsScaler object. 
        ...
        final_scaler = reduce_maxabs_scalers(self.scalers_)
        
        # Copy the attributes of the final scaler to the ParMaxAbsScaler object
    ...
    def get_partial_fit(self) -> MaxAbsScaler:
        '''
        ...
        Returns:
            tmp_scalers (MaxAbsScaler): The partial fitted scaler.
        '''
    ...
    def set_partial_fits(self, partial_fits: List[MaxAbsScaler]) -> None:
        '''
        Gather the partial fitted scalers (from the different chunks of data) into the 'scalers_' list.
        
        Args:
            partial_fits (List[MaxAbsScaler]): A list of partial fitted scalers.
    ...
    def print(self, use_parallel: bool = True) -> None:
        ...
        kind_scaler = "pmabsscaler" if use_parallel else "mabsscaler"
    ...
    def fit(self, block_reader: Union[Optional[BlockReader], None] = None, 
        ...
        if num_workers == 1 and IO_workers == 1:
            ...
            t2 = time()
            _Mneme_logger.benchmark(f"Sequential Times of Standalone MaxAbsScaler -> Fit: {t1-t0:.6f} | Reduce: {t2-t1:.6f}")
            ...
            t2 = time()
            _Mneme_logger.benchmark(f"Parallel Times of Standalone MaxAbsScaler -> Fit: {t1-t0:.6f} | Reduce: {t2-t1:.6f}")
    
    
    def _partial_fit(self, index: int, args: tuple) -> MaxAbsScaler:
        '''
        Perform fitting on a specific block of data.

        This method loads a block of data, fits a MaxAbsScaler on it and returns the fitted scaler. The block of data 
        ...
        Returns:
            MaxAbsScaler: The scaler fitted to the block of data.
        '''
        ...
        if block_offsets[index] == block_offsets[0]:
            _Mneme_logger.benchmark(f"[INFO] Indicative time for loading the data of a block [Standalone ParMaxAbsScaler]: {t1-t0:.6f}")
        
        # Fit a MaxAbsScaler on the loaded chunk and return the scaler
        scaler = MaxAbsScaler(copy = self.copy)
    ...
    def _load_chunk(self, training_file: str, block_size: int, columns: List[str], num_idxs: List[str], 
        ...
        def _compute_features(imputer: ParImputer, num_idxs: List[str], columns: List[str]) -> List[str]:
            '''
            This function determines the union of features from the ParImputer object and the num_idxs of the ParMaxAbsScaler 
            ...
            Returns:
                List[str]: A list containing the union of features from the ParImputer and ParMaxAbsScaler in the order they appear.
            '''
            ...
            ordered_list = []
            # Create a list containing the union of features from the ParImputer and ParMaxAbsScaler 
